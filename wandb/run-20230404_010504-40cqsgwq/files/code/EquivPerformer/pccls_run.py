...
def train_epoch(epoch, model, loss_fnc, dataloader, optimizer, scheduler, FLAGS):
    ...
    for i, (g, y) in enumerate(dataloader):
        ...
        pred = model(g)
        loss = smoothing(pred, cls, FLAGS.num_class)
...
def test_epoch(epoch, model, loss_fnc, dataloader, best_acc, FLAGS, dT=None):
    ...
    with torch.no_grad():
        for i, (g, y) in enumerate(dataloader):
            ...
            pred = model(g)
            loss = smoothing(pred, cls, FLAGS.num_class)
            loss_epoch += loss / len(dataloader)
            pred, cls = pred, cls
...
def get_parameter_number(model):
    ...
    tranable_num = sum(p.numel() for p in model.parameters() if p.requires_grad)

    return f'Total Params: {total_num}, trainable params: {tranable_num}'
...
def main(FLAGS, UNPARSED_ARGV):
    ...
    # Optimizer settings
    optimizer = optim.AdamW(model.parameters(), lr=FLAGS.lr, weight_decay=5e-3)
    ...
    if FLAGS.kernel:
        ...
        #scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[100, 300, 500], gamma=0.5)
        #scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=10*40, eta_min=1e-4)
        scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10 * 40 * 6, eta_min=5e-4)
    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        test_loss, test_acc, current_best = test_epoch(epoch, model, task_loss, test_loader, current_best, FLAGS)
...
if __name__ == '__main__':
    ...
    FLAGS.num_class = 40 if FLAGS.data_name == 'ModelNet40' else 15
