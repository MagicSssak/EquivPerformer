...
def main(FLAGS, UNPARSED_ARGV):
    ...
    # model name
    name = f'{FLAGS.num_points}_{FLAGS.num_random}_batch{FLAGS.batch_size}_{FLAGS.siend}_{FLAGS.num_layers + 1}_{FLAGS.num_channels//FLAGS.div}'

    # Save path
    save_path = os.path.join(FLAGS.save_dir, FLAGS.name + name + '.pt')
    ...
    if FLAGS.kernel:
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40, 100], gamma=0.33)
    else:
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[80, 160, 500], gamma=0.33)
...
if __name__ == '__main__':
    ...
    name = f'Golden_{FLAGS.num_points}_{FLAGS.num_random}_{FLAGS.data_str}_{FLAGS.batch_size}_{FLAGS.head}_{FLAGS.num_channels}_{FLAGS.num_layers}'
    wandb.init(project='ModelNet40', name=name, config=FLAGS, entity='orcs4529')
    wandb.save('*.txt')
    # Where the magic is
    try:
