...
from equivariant_attention.vn_layers import *
import time
...
class GMABSE3(nn.Module):
    ...
    def forward(self, v, k: Dict = None, q: Dict = None, G=None, **kwargs):
        ...
        with G.local_scope():
            ...
            Q = fiber2head(q, self.n_heads, self.f_key, squeeze=True)  # [edges, heads, channels](?)
            K = fiber2head(k, self.n_heads, self.f_key, squeeze=True)  # [nodes, heads, channels](?)

            q1, q2 = Q.shape[-1], Q.shape[-2]
            k1, k2 = K.shape[-1], K.shape[-2]
            Q = Q.view(batch_size, L, q2, q1)  # BxLxHxd
            K = K.view(batch_size, L, k2, k1)
            ...
            if self.kernel:
                ...
                v = v.view(batch_size, self.n_heads * chan, L, -1)  # B H*C L sum(d_out) (want BC3N)
                ...
                A = torch.matmul(Q, K.transpose(-1, -2).contiguous())/np.sqrt(self.f_key.n_features)
                # should be BxHxLxL
                ...
                # skip and norm
                #skip = v.view(batch_size, self.n_heads * chan, L, -1).permute(0, 2, 1, 3)
                #layer_norm = VNLayerNorm(chan * self.n_heads).cuda()
...
class GSE3Res(nn.Module):
    ...
    def __init__(self, f_in: Fiber, f_out: Fiber, edge_dim: int = 0, div: float = 4,
        ...
        self.GBN = nn.ModuleDict()
        self.GLNR = nn.ModuleDict()
        self.dim = f_mid_out[0]

        # for k in f_mid_out.keys():
        #     self.GBN[f'{k}'] = VNLinearAndLeakyReLU(f_mid_out[k], f_mid_out[k])
        #     self.GLNR[f'{k}'] = VNLinear(f_mid_out[k], f_mid_out[k])
        ...
        #self.bn0 = VNLinearAndLeakyReLU(self.)
        # Projections
        ...
        self.GMAB['attn'] = GMABSE3(self.f_mid_out, self.f_mid_in, n_heads=n_heads,
                                    ...
                                    kernel_channel=self.kernel_channel)
    ...
    def forward(self, features, G, **kwargs):
        ...
        # B*N, C, d   want B, C, d, N
        # for k in z.keys():
        #     z[f'{k}'] = z[f'{k}'].reshape(batch_size, self.dim, -1, 256)
        #     z[f'{k}'] = z[f'{k}']
        #     z[f'{k}'] = self.GBN[f'{k}'](z[f'{k}']).reshape(batch_size*256, self.dim, -1)
        ...
        #for k in z.keys():
        #    z[f'{k}'] = z[f'{k}'].reshape(batch_size, self.dim, -1, 256)
        #    z[f'{k}'] = self.GBN[f'{k}'](z[f'{k}'])
        #    z[f'{k}'] = self.GLNR[f'{k}'](z[f'{k}']).reshape(batch_size*256, self.dim, -1)
