...
class VNLinear(nn.Module):
    ...
    def forward(self, x):
        '''
        x: point features of shape [B, C, 3, N_samples, ...]
        '''
...
class VNLayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        B, N, C, P = x.shape
        # x = x.view(B, N, C, P)
        norm = torch.norm(x, dim=-1) + EPS  # B N C, vector norm
        norm_ = self.ln(norm).unsqueeze(-1)  # B N C 1, layer norm
        norm_ = torch.matmul(norm_, torch.ones((1, P)).cuda())
        # LayerNorm Feeds
        feed = x / norm.unsqueeze(-1)  # unit vector
        x = feed * norm_  # output
